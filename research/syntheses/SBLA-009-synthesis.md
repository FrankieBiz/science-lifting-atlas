...
- Date: 2026-09-13; revised 2026-09-13 in the R1 bounded remediation and 2026-09-15 in
  the SBLA-010 R2 bounded remediation
...
- Second revision: SBLA-010 R2 bounded remediation against
  ...
  `3428d3feb9b58bb150ac4c93375d868cabc5dc3f5597e83e50503b30a47a12b5`). Every change is
  listed in §9. The artifact version stays 2.0.0 because that report's finding N-3
...
- **Bench angle, grip width and range of motion change which part of the muscle is
  loaded**, not merely how much (G0103, G0348, G0288, G0381, G0090). G1944, obtained in
  the R2 remediation, is the most portion-resolved of these and it complicates the
  picture rather than confirming it: across twelve variations crossing three
  inclinations with two grip types and two widths, at each cell's own 12RM, it reports
  **no significant sternocostal difference between the three inclinations within matched
  pronated grips** and **no clavicular advantage for the incline at a matched wide
  pronated grip**. Its apparent regional pattern is carried by cells that differ in grip
  and in absolute load as well as in angle, so it is used here for its null statements
  and not for a regional direction (C-16, C-17).
...
| C-14 | The 2023 meta-analysis contradicts itself on the **direction** of the type-of-exercise sternal difference. Its Results text states twice that "there is a greater activation in the sternal pectoralis **in the variable exercise** (SMD = 4.04)", while its Abstract states that "greater activations are also seen in the original bench press vs. the comparisons" and its Featured Application that "BP is the one that most involves the pectoralis major". The mechanism is that the paper's sign convention **inverts between its contrast families**: positive favours the horizontal bench press in the inclination family and the comparator exercise in the type-of-exercise family | G1903 (internal)                           | No direction is taken from this source. Claim 21's assertion that the sternal difference favoured the bench press is withdrawn and the contrast is stated without a direction, matching the extraction and this synthesis. Recorded, not resolved: the source cannot settle its own direction |
| C-15 | Two further internal defects in the same source. (a) Two distinct results both carry **SMD = 4.53** and are separated only by their full signature: Table 3's clavicular type-of-exercise row (95 % CI −4.22 to 13.27, I² 94.9 %, t 1.65, p 0.198) and the Figure 7 other-exercises-versus-push-up comparison (95 % CI 4.40 to 7.65). (b) The Abstract reports a search of "four databases" while Methods §2.1 and Appendix A name and reproduce exactly **three**                                                                                                                                                                                                                             | G1903 (internal)                           | Every citation of SMD 4.53 in this pass now carries its confidence interval. The three-database reading is used because the Methods and the Appendix agree on it; the Abstract's count is unsupported anywhere in the paper                                                                   |
| C-16 | Bench inclination and the sternocostal portion: this synthesis has treated the sternocostal decrease with inclination as the consistent half of the picture, and the twelve-variation study recovered in the R2 remediation reports **no significant sternocostal difference across −15°, 0° and +30° within matched pronated grips**, and no clavicular advantage for the incline at a matched wide pronated grip                                                                                                                                                                                                                                                                             | G1944 vs G0103, G0348, G1903               | Claim 14's first qualifier is weakened: the sternocostal decrease is the **more** consistent finding, not a universal one, and G1944 is cited on the claim with role `contradicts`. Certainty stays `low` and direction stays `mixed`; nothing was raised                                     |
| C-17 | G1944 contradicts its own headline. Its Abstract concludes that "the bench press exercise performed with a wide pronation grip at 0° can maximize the activation of the three heads of the PM", while its Table 3 lists that same position among the positions of **minimum** activation for the clavicular head and its Practical Application states that a closer grip recruits the clavicular head better at 0°                                                                                                                                                                                                                                                                             | G1944 (internal)                           | Table 3 and the Results text are the primary record and are what the extraction relies on. The Abstract conclusion is recorded as an author interpretation and is not extracted as a finding                                                                                                  |
...
- **159 ClinicalTrials.gov registrations** matched the intervention-and-outcome
  ...
  gap is upstream of publication bias, at the point where these routes retrieved no
  such study, registered or published.
...
- **L-4. Forty-six of 89 included sources were read as abstracts only.** Their facts
...
## 9. What the SBLA-010 R2 bounded remediation changed in this synthesis
...
This is the one bounded remediation the stop rule allows after that failed review, and
it is followed by one complete-artifact recheck.

| Finding       | Change                                                                                                                                                                                                                                                                                                                                                                  |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **N-1**       | G1944 was obtained and read. Its own recorded publisher URL served the complete CC BY PDF; the 23-byte "200" of 2026-09-13 was an unfollowed redirect. It is now included, and §2.4 records what it reports. The included count moves 88 → 89, awaiting-full-text 26 → 24, excluded 1,842 → 1,843, and §7 L-4 is recomputed to forty-six of **89**.                     |
| **N-1**       | G1087 was obtained on the same day, its DOI identity verified at Crossref and against its SportRxiv landing page, read in full, and **excluded** under `E-OUT-1`: it is a survey of exercise naming with no pectoralis outcome. Recovering full text is not a reason to include a record, and the two recovered records went in opposite directions.                    |
| **N-2**       | New contradiction-map row **C-14**. The 2023 meta-analysis cannot settle the direction of its own type-of-exercise sternal difference, because its sign convention inverts between contrast families. No direction is taken from it, and claim 21's assertion that the difference favoured the bench press is withdrawn.                                                |
| **M-13, new** | New contradiction-map row **C-15** records two further defects in that source: two distinct results both carrying SMD = 4.53, separated only by their confidence intervals, and an Abstract claiming four databases where the Methods and Appendix name three.                                                                                                          |
| **new**       | New contradiction-map rows **C-16** and **C-17**. C-16 records that G1944 contradicts this synthesis's own "sternocostal falls as the bench is raised" reading within matched grips; claim 14's first qualifier is weakened accordingly and G1944 is cited on it with role `contradicts`. C-17 records that G1944 contradicts its own Abstract for the clavicular head. |
| **N-4 class** | §4 no longer says the gap is "at the point where nobody has run the study". It now says these routes retrieved no such study, registered or published. The universal form was the same defect the review found in the bench-press draft.                                                                                                                                |
| **M-3**       | The header now records the second revision, and the companion-version line states that the evidence packet carries no version field because its schema does not own one.                                                                                                                                                                                                |

**Certainty movement in this round: none upward.** Claim 14 stays `low` / `mixed` with a
weakened qualifier; claim 21 stays `very-low` and loses an assertion. One source was
added and it entered as contradicting evidence.
